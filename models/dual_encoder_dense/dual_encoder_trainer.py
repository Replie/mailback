...
def train_main(hparams):
    ...
    save_every_n_batches = hparams.save_every_n_batches
    ...
    for epoch in range(hparams.nb_epochs):
        ...
        # save model
        save_model(saver=saver, hparams=hparams, sess=sess, epoch=epoch, step='FINAL')
...
def evaluate_recall(y, y_test, k=1):
    ...
    for predictions, label in zip(y, y_test):
        if label in predictions[:k]:
            num_correct += 1
    print('\nrecall', num_correct / num_examples, k)
...
def save_model(saver, hparams, sess, epoch, step):
    print('\nsaving model...')

    # create path if not around
    model_save_path = os.path.join(hparams.model_save_dir, hparams.exp_name, 'epoch_{}/step_{}'.format(epoch, step))
